...
There are 7,014 Candidates in the data set
...
## Answer 2
```{r}
party_incumbent_promote = fb_ads |> filter(office == "State House" | office == "State Senate") 


promote_prop = party_incumbent_promote |>
  group_by(party, incumbency) |>
  summarise(
    promote_prop = mean(ad_tone_promote, na.rm = TRUE)) 

knitr::kable(promote_prop, digits = 3, col.names = c("Party", "Incumbency", "Promote_Prop"))

```
Democratic incumbents sponsored the most promoting ads on average
...
```{r}
impressions_millions = (total facebook ad impressions_millions) 
save the resulitng data set back as fb_ads
...
plot_q3 <- ggplot(fb_ads_house, aes(x = impressions_millions)) +
  geom_histogram( fill = "blue", color = "black") +
  labs(
    title = "Facebook Ad Impressions (in Millions) for US House Races",
...
print(plot_q3)
```
The Histogram above is steeply right skewed and shows that almost none of US House races have over 10 million add impressions 
\newpage
...
```{r}
fb_top_plot = fb_ads |> 
  ggplot(fb_ads_house, aes(x = impressions_millions)+
                                 barplot(scale_fill_manual(values = c(R = "red", D = "blue")), + 
           labs( title =  "Top 15 House Canidate Facebook Ad Impressions, 2017-2018", 
                 x = impressions_millions, y = cand_name_last)
...
top_15_house <- top_15_house |>
  mutate(cand_name_last = reorder(cand_name_last, impressions_millions))

fb_top_plot <- ggplot(top_15_house, aes(x = impressions_millions, y = cand_name_last, fill = party)) +
  geom_bar(stat = "identity") + 
  scale_fill_manual(values = c(R = "red", D = "steelblue")) + 
  labs(
